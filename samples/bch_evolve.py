...
Ucoeff = np.array([init_vec.inner(vec) for vec in uevecs]) # < vec | init_vec >
Hcoeff = np.array([init_vec.inner(vec) for vec in bchevecs]) # < vec | init_vec>
...
for t in range(10):
    ...
    Hvec = sum( np.exp(-1.j/hbar * bchevals[i] * t) * Hcoeff[i] * bchevecs[i] for i in range(dim))        
